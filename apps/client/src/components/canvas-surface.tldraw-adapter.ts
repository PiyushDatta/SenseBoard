...
const clampBoardElementForOps = (
  ...
  let sanitizedZIndex: number | undefined;
  if (element.zIndex === undefined) {
    sanitizedZIndex = undefined;
  } else if (isFiniteNumber(element.zIndex)) {
    const normalized = Math.round(clampNumber(element.zIndex, -100000, 100000));
    if (normalized !== element.zIndex) {
      clampReasons.add('upsert:zindex');
      counter.count += 1;
    }
    sanitizedZIndex = normalized;
  } else {
    sanitizedZIndex = undefined;
    clampReasons.add('upsert:zindex');
    counter.count += 1;
...
export const guardBoardOpsEnvelope = (
  ...
  if (hasEnvelope) {
    ...
    const hasNumericSchema = isFiniteNumber(schemaField);
  ...
  const skipReasonsRecord = Object.fromEntries(
    Array.from(skipReasons.entries()).sort(([left], [right]) => left.localeCompare(right)),
  );
  const telemetry: BoardOpsGuardTelemetry = {
    ...
    skippedTopLevelOps: skippedTopLevel,
    skippedNestedOps: skippedNested,
  ...
  const guardIntervened = envelopeInvalid || fallbackApplied || telemetry.clampedOps > 0 || telemetry.skippedOps > 0;
  if (guardIntervened) {
